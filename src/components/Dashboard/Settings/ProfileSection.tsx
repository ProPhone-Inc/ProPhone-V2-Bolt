...
import { User, Mail, Phone, Globe, Camera, ToggleLeft as Google, Facebook, Lock } from 'lucide-react';
...
export function ProfileSection({ userData, setUserData }: ProfileSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update in database
      if (userData?.id) {
        await db.users.update(userData.id, updatedUserData);
      }
        ...
        {isEditing && (
          <div className="flex justify-end space-x-3">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-4 py-2 bg-gradient-to-r from-[#B38B3F] via-[#FFD700] to-[#B38B3F] text-black font-medium rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              {isLoading ? (
